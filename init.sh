...
if [ "$NODE_ENV" != "$PROD" ]; then
    ...
    docker run --name express-rest-dev --link mongo-server:mongodb.data.server --link mongo-server-test:mongodb-test.data.server -v `pwd`:/usr/src/app -p 3000:3000 express-rest-base:latest /bin/bash
fi
